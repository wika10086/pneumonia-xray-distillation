...
这是一个面向 PyTorch 初学者的基础图像识别与知识蒸馏项目。当前环境已经配置好，可以使用 NVIDIA GPU 训练模型。

本机当前已准备好 **PneumoniaMNIST 224x224** 数据集，并在训练集中加入了经过去重的 Epic Chittagong 胸片。这个项目只用于学习和实验，不能用于真实临床诊断。

GitHub 仓库只包含源码和说明文档。数据集、模型权重、训练输出、虚拟环境和打包好的 exe 均需在本机另行准备；单独克隆源码不能直接运行预测界面。
